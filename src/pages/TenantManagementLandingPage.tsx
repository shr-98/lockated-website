...
const TENANT_NAV_OFFSET_PX = 70

/**
 * `public/tenant-management.html` is a self-contained design with its own fixed nav,
 * fonts, and inline `<script>` (counters, reveal observer, walkthrough/feature tabs,
 * scroll-driven team panels, industry modal). We fetch it, inject styles + body, then
 * execute the inline scripts so the original behavior runs unmodified.
 */
const TENANT_ISOLATION_CSS = `
.tenant-mgmt-root {
  position: relative;
  isolation: isolate;
}
html:has(.tenant-mgmt-root) {
  scroll-padding-top: ${TENANT_NAV_OFFSET_PX}px;
  scrollbar-gutter: stable;
}
.tenant-mgmt-root section[id] {
  scroll-margin-top: ${TENANT_NAV_OFFSET_PX + 4}px;
}
.tenant-mgmt-root #nav {
  z-index: 10050;
}
...
.tenant-mgmt-root .reveal.visible {
  opacity: 1 !important;
...
export default function TenantManagementLandingPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const ts = Date.now()
        const url = `/tenant-management.html?ts=${ts}`
        const res = await fetch(url, { cache: 'no-store' })
        if (!res.ok) throw new Error(`HTTP ${res.status} loading ${url}`)
        const text = await res.text()

        if (!/id=["']?nav["']?/.test(text) || !/Tenant Management/i.test(text)) {
          throw new Error(
            'Loaded /tenant-management.html does not look like the tenant page. Ensure the file exists in public/.',
          )
        }
        ...
        if (cancelled) return
        setLoadError(e instanceof Error ? e.message : 'Failed to load Tenant Management content')
      }
    }
  ...
  useEffect(() => {
    ...
    if (externalScripts.length === 0) {
      ...
      for (const { src } of externalScripts) {
        const existing = document.querySelector<HTMLScriptElement>(
          `script[data-tenant-external="${src}"]`,
        )
        if (existing) {
          onDone()
          continue
    ...
    const cleanupLeadForm = hookLeadForm(root, 'tenant-management')
    return () => {
  ...
  return (
    <div ref={rootRef} className="tenant-mgmt-root min-h-dvh bg-[#F6F4EE]">
      {headLinks.map((l) => (
        <link
          key={`${l.rel}:${l.href}`}
          rel={l.rel}
      ...
      {loadError ? (
        <div style={{ padding: 24 }}>Tenant Management error: {loadError}</div>
      ) : bodyHtml ? (
